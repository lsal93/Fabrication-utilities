...
schemasrem = [f'*#{path_value}' for path_value in dir_path.values()]
schemasrem = [
    (
        f'*#{path_value}'
...
removeapp = App(
    label='Fabrication remove steps',
    path='removeapp',
    category='Fabrication facilities',
    description='App to search remove fabrication steps.',
    readme="""
    This app is intended to navigate around the ecosystem of clean room fabrication
    possible remove steps. At the beginning you can see all the fabrication steps
    available in nomad and than through the filters you can specialize the research per
    single remove technique. Navigation across different techniques is not allowed.
    """,
    search_quantities=SearchQuantities(include=schemasrem),
    columns=columns_to_use
    filters_locked={'section_defs.definition_qualified_name': dir0},
    menu=Menu(
        title='Remove steps',
